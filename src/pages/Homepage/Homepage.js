...
import { Input, Menu, Accordion, Card } from "semantic-ui-react";
import axios from "axios";

const Homepage = () => {
  const [ingredient, setIngredient] = useState("");
  const [isAlcohol, setIsAlcohol] = useState(false);
  const [isAccordionActivated, setIsAccordionActivated] = useState(false);
  ...
  const handleAccordion = (event) => {
    setIsAccordionActivated(!isAccordionActivated);
  };
  ...
  const handleAlcoholChange = async (data) => {
    await setIsAlcohol(data);
    let url = isAlcohol
  ...
  //load drinks based on ingredient filter
  ...
  useEffect((setIsAlcohol) => {
    handleAlcoholChange(ingredient);
  });

  const handleIngredientChange = (value) => {
    let url = `http://www.thecocktaildb.com/api/json/v1/1/filter.php?i=${value}`;
    loadDrinks(url);
  };

  const handleInputChange = (event) => {
    const value = event.target.value;
    let url = `http://www.thecocktaildb.com/api/json/v1/1/search.php?s=${value}`;
    loadDrinks(url);
  };

  //helper function for loading drinks
  function loadDrinks(url) {
    axios
      .get(url, {
  ...
  return (
    <>
      <header className="header text-center">
        <h2 className="mt-3">Welcome, Usman!</h2>
        <Input
          size="big"
          icon="search"
          placeholder="Search..."
        ...
        />
        <div>
          <h3 className="text-white mt-3">Filter by</h3>
          <Accordion as={Menu} size="large" vertical>
            <Menu.Item>
              <Accordion.Content
                active={isAccordionActivated}
                content={
                  <Alcoholic
                    onAlcoholChange={(val) => setIsAlcohol(val)}
                    isAlcohol={isAlcohol}
                  />
                }
              />
            ...
            <Menu.Item>
              ...
              <Accordion.Content
                ...
                content={
                  <Ingredients
                    onIngredientChange={(val) => setIngredient(val)}
                  />
                }
              />
            </Menu.Item>
      ...
      <div className="drinks">
        <h2 className="mt-3 text-center">Drinks</h2>
        <div className="row text-center justify-content-center animated fadeIn">
          {drinks.map((drink) => (
            <div key={drink.idDrink} className="col-md-4 col-lg-3 mb-3 ">
              <Card
                onClick={() => handleModalShow(drink)}
                image={`${drink.strDrinkThumb}/preview`}
                header={drink.strDrink}
                meta={`${drink.strAlcoholic}, ${drink.strCategory}, ${drink.strGlass}`}
